...
class imgFinder:
    ...
    def getTag(self, path):
        tags = piexif.load(self, path)  # return exif tags
    ...
    # 유사 이미지 찾기
    def findImg(self, folder_path, query, save_dir):
        # 대상 이미지 불러오기
        query_img = cv2.imread(query, cv2.IMREAD_COLOR)
        query_img = cv2.resize(query_img, (400, 400)) # 이미지 사이즈 바꾸고
        query_hash = self.img2hash(query_img) # 해쉬 이미지로 변환
        cv2.imshow('query', query_img)  # 대상 이미지 띄워놓기

        qTag = self.getTag(query) # 쿼리 이미지 태그
        self._createFolder(save_dir) # 저장할 폴더 생성
        ...
        for path in folder_path:
            ...
            img_hash = self.img2hash(reimg)
            dst = self.hamming_distance(query_hash, img_hash) / 256 # hamming distance 비율로 바꾸기 (0 ~ 1)

            if dst < 0.20: # hamming distance 가 80퍼 이상 유사한 이미지를 찾는다.
                print(path, dst)
                cv2.imshow(path, reimg)

                tag = self.getTag(path) # 유사 이미지의 태그 정보
...
test = imgFinder()
test.findImg(dir_path, test, save)
